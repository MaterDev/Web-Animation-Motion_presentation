...
export function deviceCard(apps) {
  ...
  const host = { canvas, get ui() { return ui; }, pointer: { x: -1, y: -1, down: false } };
  const buttons = apps.map((a) => $('dv-app-' + a.id));
  const caption = (id) => apps.forEach((a) => { const c = $('dv-cap-' + a.id); if (c) c.hidden = a.id !== id; });
  const open = (id) => { if (id === curId || !dev) { pending = id === curId ? null : id; return; } pending = id; loading.hidden = false; loading.textContent = 'opening ' + apps.find((a) => a.id === id).name + '…'; };
  ...
  return card({ name: 'device', el, init() { dev = this.__dev; ui = makeUI(canvas, DUO.w, DUO.h);
      uiPointer(ui, (p) => { host.pointer.x = p.x; host.pointer.y = p.y; hover = ui.at(p.x, p.y); canvas.style.cursor = hover ? 'pointer' : (cur && cur.cursor) || 'default'; if (cur && cur.move) cur.move(p, hover, drag); },
        ...
        (p) => { drag = false; host.pointer.down = false; if (p && cur && cur.up) cur.up(p, press, press && ui.at(p.x, p.y) === press); press = null; });
      const want = (location.hash || '').slice(1); pending = apps.some((a) => a.id === want) ? want : apps[0].id; },
    frame(t, dt, now) { if (!ui) return; if (pending) { if (loading.hidden) { loading.hidden = false; loading.textContent = 'opening ' + apps.find((a) => a.id === pending).name + '…'; return; } swap(); loading.hidden = true; }
