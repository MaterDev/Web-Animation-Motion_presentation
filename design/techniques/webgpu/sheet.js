...
import { markCards } from './marks.js';
import { fluidCards } from './fluids.js';
import { populationCards } from './population.js';
import { proceduralCard } from './procedural.js';
import { opticsCard } from './optics.js';

const cards = [clockCard(), ...loopCards(), ...markCards(), ...fluidCards(), ...populationCards(), proceduralCard(), opticsCard()];
...
(async () => {
  ...
  let n = 0; const tick = () => { n++; if (n % 20 === 0) { $('instrument-bytes').textContent = fmtB(bytes.frameState); $('instrument-uni').textContent = fmtB(bytes.frameUni); $('loop-bytes').textContent = fmtB(bytes.frameState) + ' state'; $('loop-count').textContent = (160000 + 36000).toLocaleString(); } requestAnimationFrame(tick); }; tick();
  void demo;
})();
