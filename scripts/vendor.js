   ...
   actually on screen there. */
{
  ...
  /* How much wider than its own viewport each level is cut. 1.6 rather
     than 1.0 because a layer has to be complete from the moment it
     becomes the visible one, not from the moment the camera settles on
     it, and the crossfade starts early. It is emitted with the map
     because the stage's crossfade needs the same number to know how
     early it may start — two files agreeing by retyping it is how they
     stop agreeing. */
  const MARGIN = 1.6;
  ...
  const layers = LEVELS.map((lv, i) => {
    ...
    if (i > 0) {
      /* HALF-widths. Writing W/k here rather than W/k/2 was a two-times
         ...
         MARGIN is declared above, with the reason it is not 1.0. */
