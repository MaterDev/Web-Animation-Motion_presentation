// @ts-nocheck -- the sheets' runtime, copied verbatim apart from the three changes below; it was never typed
/* ── WAM clock, as a module the deck's demos own ────────────────
   A copy of design/system/wam.js, the sheets' shared runtime, changed in
   exactly three ways so a demo can live inside a slide and leave again:

     · createWAM() returns an INSTANCE, not a window global: two demos
       with the same clock name no longer collide in one registry
     · dispose() removes every listener, observer and frame the instance
       started (the sheet version never unmounts, so never needed to)
     · filmstrip() looks its stage up in `root` (the demo's shadow root)
       rather than document

   Everything else, including render(t), the cost and fps meters and the
   reduced-motion poster, is the sheet's code unchanged. */
/** @param {{ root?: Document | ShadowRoot | Element }} [opts] */
export function createWAM({ root = document } = {}) {

  const mq = matchMedia('(prefers-reduced-motion: reduce)');
  const demos = {};
  /* every listener, observer and frame this instance starts, so one
     dispose() stops all of it when the slide unmounts */
  const undo = [];
  ...
  function clock(name, { render, dur = 6000, el, poster = 1 }) {
    ...
    const onVis = () => { if (document.hidden) pause(); else if (!mq.matches) play(); };
    ...
    const onMq = () => { if (mq.matches) { pause(); draw(poster); } else play(); };
    mq.addEventListener('change', onMq);
    undo.push(() => mq.removeEventListener('change', onMq));
    return api;
  }
  ...
  function filmstrip(name, n = 12) {
    ...
    const stage = root.querySelector('[data-testid="' + name + '"]');
    if (!stage) return console.error('no stage for: ' + name);
    root.querySelector('.qc-strip')?.remove();
    ...
    const anchor = root.querySelector('.notes') || stage.closest('section') || stage;
    anchor.before(strip);
    d.render(was); if (wasPlaying) d.play();
    return strip;
  }

  function dispose() { while (undo.length) { try { undo.pop()(); } catch { /* already gone */ } } }

  return { clock, filmstrip, copyBitmaps, demos, clockRes, dispose, get reduced() { return mq.matches; } };
}
