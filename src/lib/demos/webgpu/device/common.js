...
/* extraction: `$` queries the mounted demo's shadow root, set by index.js on mount */
let scope = null;
export const setScope = (r) => { scope = r; };
export const $ = (id) => scope.querySelector(`[data-testid="${id}"]`);
...
/* extraction: the sheet never gave its device back. A mounted demo does, so the module-level
   singletons are cleared on dispose and a re-mount acquires a fresh device. Only clears them when
   `dev` is still the current device, so a late-resolving old mount cannot null a new one. */
export function releaseDevice(dev) {
  if (dev) { try { dev.destroy(); } catch {} }
  if (!dev || dev === device) { device = null; adapter = null; format = null; hasTS = false; }
  bytes.frameState = 0; bytes.frameUni = 0; cards.length = 0;
}
...
/* extraction: the WAM instance and a disposer are passed in, in place of window.WAM and a page that never unmounts */
export function run(WAM, dsp, rootEl, dur = 6000) {
  const seen = new Map();
