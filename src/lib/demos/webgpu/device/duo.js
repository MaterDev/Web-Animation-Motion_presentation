...
export function makeHardware(dev, canvas, ctx, fmt) {
  ...
  const rect = { x: 0, y: 0, w: 2, h: 2 }, bounds = { x: 0, y: 0, w: 2, h: 2 }; /* the open display, and the open body, in device pixels */
  ...
  const camera = (p) => {
    ...
    /* the swing pulls back mid-way so the whole device stays in frame while it turns and unfolds */
    const yaw = lerp(yawC, yawO, e), pitch = lerp(pitchC, 0, e), dist = lerp(closedDist, openDist(), e) * (1 + 0.3 * Math.sin(Math.PI * e));
  ...
  const hw = {
    rect, bounds, get ready() { return !!bodyT; },
    resize() { cw = canvas.width; ch = canvas.height; if (bodyT) { bodyT.destroy(); scrT.destroy(); }
      ...
      const bx = W / (d * TF * (cw / ch)) * cw / 2, by = (H * 0.5) / (d * TF) * ch / 2; bounds.x = cw / 2 - bx; bounds.y = ch / 2 - by; bounds.w = bx * 2; bounds.h = by * 2;
