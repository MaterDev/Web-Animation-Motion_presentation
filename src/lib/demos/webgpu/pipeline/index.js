...
export function mount(host) {
  ...
  (async () => {
    ...
    run(WAM, d, body, 6000);
    let n = 0, raf = 0; const tick = () => { n++; if (n % 20 === 0) { $('instrument-bytes').textContent = fmtB(bytes.frameState); $('instrument-uni').textContent = fmtB(bytes.frameUni); } raf = requestAnimationFrame(tick); }; tick();
