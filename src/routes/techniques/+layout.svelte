<script>
  ...
  import { TECHNIQUES, techniqueHref } from '$lib/sheets.js';

  let { children } = $props();

  /* The technique sheets, a top-level section of their own, in the
     talk's order. Same sidebar shape as the design section. */
  ...
  const groups = $derived([
    { label: 'Techniques', items: TECHNIQUES.map(toItem) },
  ]);

  /** @param {any} s */
  function toItem(s) {
    const href = techniqueHref(s.code);
    return {
      code: s.code,
      name: s.name,
      note: s.note,
      ...
      current: page.url.pathname === '/techniques' && (sheetParam || 'sl-06') === s.code.toLowerCase(),
    };
  }
</script>

<div class="section">
  <aside class="side" data-testid="techniques-sidebar">
